...
class Useraction extends BaseController
{
    ...
    public function prof_data()
    {
        ...
        if ($this->request->isAJAX()) {
            ...
            return $response->setHeader(csrf_header(), csrf_hash())->setBody(json_encode($data));
        }
    }
    ...
    public function update_profile()
    {
        ...
        if ($this->request->isAJAX()) {
            $user_rand_id = $jwt_decoded['id'];
            $user_data = (array) json_decode($request->getPost('user_data'));
            $user_model = new UserModel();
            $user_model->setUserData($user_rand_id, $user_data);
            return $response->setHeader(csrf_header(), csrf_hash())->setBody('{"msg":"success"}');
        }
    }
    ...
    public function pass_change()
    {
        ...
        $jwt_decoded = Useraction::session_check();
        $user_rand_id = $jwt_decoded['id'];
        $old_pass = $request->getPost('old_pass');
        $new_pass1 = $request->getPost('pass1');
        $new_pass2 = $request->getPost('pass2');
    ...
    public function cat_create()
    {
        ...
        if ($this->request->isAJAX()) {
            if ($sess_check != NULL && $sess_check != '') {
                $user_model = new UserModel();
                $un_san_data = json_decode($request->getPost('cat_data'), true);
                ...
                if ($valid->run($un_san_data, 'categoryvalid') === FALSE) {
                    ...
                    $error['msg'] = 'error';
                    return $response->setHeader(csrf_header(), csrf_hash())->setBody(json_encode($error));
                } else {
                    $aff_rows = $user_model->setCategory($cat_data);
                    if ($aff_rows != 0) {
                        return $response->setHeader(csrf_header(), csrf_hash())->setBody('{"msg":"success"}');
                    } else {
                        return $response->setHeader(csrf_header(), csrf_hash())->setBody('{"msg":"failed"}');
                    }
                }
            }
        }
