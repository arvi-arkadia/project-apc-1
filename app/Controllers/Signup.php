    ...
    */
    ...
    public function register()
    {
        ...
        if ($request->getMethod() == 'post') {
            ...
            $username_val = $u_model->getUniqueUsername($request->getPost('uname'));
            ...
            if ($validation->withRequest($this->request)->run() === FALSE || $username_val != NULL) {
                ...
                $hash_pass = password_hash($request->getPost('passwd'), PASSWORD_BCRYPT);
                $random_id = random_string('alnum', 32);
                $san_data = array(
                    'user_name' => esc($request->getPost('uname')),
                    'user_mail' => esc($request->getPost('e_mail')),
                    'user_pass' => $hash_pass,
